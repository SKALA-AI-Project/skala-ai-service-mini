...
class TrlAnalysisNode:
    ...
    indicator_keywords: dict[str, tuple[str, ...]] = {
        "patent": ("patent", "uspto", "출원", "특허", "jp", "kr"),
        "conference": ("isscc", "hot chips", "arxiv", "학회", "논문"),
        "hiring": (
            "linkedin",
            "recruit",
            "hiring",
    ...
    def _infer_trl(
    ...
    ) -> tuple[int, Literal["confirmed", "estimated"], Literal["high", "medium", "low"], str | None, int]:
        """5개 간접지표와 양산·출하 신호를 조합해 TRL과 신뢰도를 결정한다."""
        text_blob = " ".join(
            f"{item['query']} {item['title']} {item['content']}".lower()
            for item in items
        )
        signal_count = self._count_indicator_signals(items)
        ...
        if production_count > 0:
            ...
            return trl_confirmed, "confirmed", confidence, None, signal_count

        # ── 2단계: 지표 조합별 규칙 ─────────────────────────────────────
        has_patent = self._detect_indicator(text_blob, "patent")
        has_academic = self._detect_indicator(text_blob, "academic")
        has_hiring = self._detect_indicator(text_blob, "hiring")
        has_ir = self._detect_indicator(text_blob, "ir")
        has_partnership = self._detect_indicator(text_blob, "partnership")
        ...
        if has_academic and has_patent and not has_hiring:
            ...
            return 3, "confirmed", confidence, None, signal_count
        ...
        limitation = (
            f"Samsung의 제한적 공개 정책으로 인해 실제 기술 성숙도와 ±1~2단계 오차가 있을 수 있음"
            if company == "Samsung"
            else "TRL 4~6 구간은 간접지표 기반 추정이며 실제값과 ±1~2단계 오차가 있을 수 있음"
        )
        return trl, basis, confidence, limitation, signal_count
    ...
    def _detect_indicator(self, text_blob: str, indicator_type: str) -> bool:
        ...
        if self._trl_config is not None:
            ...
            fallback_map: dict[str, tuple[str, ...]] = {
                "patent": self.indicator_keywords.get("patent", ()),
                "academic": self.indicator_keywords.get("conference", ()),
                "hiring": self.indicator_keywords.get("hiring", ()),
                "partnership": self.indicator_keywords.get("partnership_supply", ()),
                "ir": self.indicator_keywords.get("ir_language", ()),
            }
            keywords = fallback_map.get(indicator_type, ())
        return any(kw in text_blob for kw in keywords)

    def _count_indicator_signals(self, items: list[SearchResult]) -> int:
        """서로 다른 지표에서 최소 1개 키워드라도 탐지되면 신호 1개로 계산한다."""
        text_blob = " ".join(
            f"{item['query']} {item['title']} {item['content']}".lower() for item in items
        )
        return sum(
            any(keyword in text_blob for keyword in keywords)
            for keywords in self.indicator_keywords.values()
        )

    def _count_production_signals(self, items: list[SearchResult]) -> int:
        """양산·출하 관련 키워드가 몇 개 매칭되는지 반환한다."""
        text_blob = " ".join(
            f"{item['query']} {item['title']} {item['content']}".lower() for item in items
        )
        keywords = (
    ...
    def _apply_company_disclosure_penalty(
        ...
        config.trl.samsung_confidence_penalty=False 이면 패널티를 적용하지 않는다.
        """
        penalty_enabled = (
